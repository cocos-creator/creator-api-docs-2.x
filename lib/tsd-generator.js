...
(function initBasicTypeMap () {
    for (let validType of Object.values(basicTypeMap)) {
        basicTypeMap[validType] = validType;
    }
    for (const globalType of IgnoreTypes) {
        basicTypeMap[globalType] = globalType;
    }
})();

const DOM_RE = /^(?:(?:HTML|DOM)\w*)?Element$|XMLHttpRequest/;
function transforType(type) {
    ...
    if (type[0] === '[' && type[type.length - 1] === ']') {
        return transforType(type.slice(1, -1)) + '[]';
    }
    if (type.includes('|')) {
...
function computeModules(ast) {
    ...
    // get module name
    for (let moduleName in ast.modules) {
        if (!REG_IDENTIFIER_WITH_DOT.test(moduleName)) {
            // console.log(moduleName);
            continue;
        }
        let module = ast.modules[moduleName];
        modules[moduleName] = {
    ...
    function formatMember (member) {
        ...
        if (member.return) {
            ...
            if (type) {
                ...
                console.warn(`Missing return type of "${member.module}.${member.clazz}.${member.name}"`);
            }
        }
    }
...
function findRefIn (module, name) {
    if (name.startsWith(module.name + '.')) {
        name = name.slice(module.name.length + 1);
    }
    if (module.classes[name] || module.nestClasses.find(x => x.name === name)) {
        return name;
    }
    let found = module.nestClasses.find(x => x.name.endsWith(name));
    if (found) {
        return found.name;
    }
    if (name.includes('.')) {
        let found = module.nestClasses.find(x => name.endsWith('.' + x.name));
        if (found) {
            return found.name;
        }
    }
...
function fixRef (name) {
    let found = findRefIn(parsingModule, name);
    if (found) {
        return found;
    }
    ...
    for (var moduleName in modules) {
        ...
        let found = findRefIn(module, name);
        if (found) {
    ...
    console.warn('Can not find "%s"', name);
    return name;
}
...
function startClass(clazz) {
    ...
    function generateOut(clazz) {
        ...
        var decl = 'export class ';
        if (clazz._raw && clazz._raw.typescript) {
            decl += clazz._raw.typescript;
        }
        else {
            decl += clazz.name;
        }
