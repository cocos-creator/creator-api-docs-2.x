...
##### Properties
  ...
  - [`isPlaying`](#isplaying) `Boolean` 该音频剪辑是否正播放（只读）。<br/>
注意：Native 平台暂时不支持 isPlaying。
  - [`clip`](#clip) `AudioClip` 默认要播放的音频剪辑。
  - [`volume`](#volume) `Number` 音频源的音量（0.0 ~ 1.0）。
  - [`mute`](#mute) `Boolean` 是否静音音频源。Mute 是设置音量为 0，取消静音是恢复原来的音量。
  - [`loop`](#loop) `Boolean` 音频源是否循环播放？
  - [`playOnLoad`](#playonload) `Boolean` 如果设置为true，音频源将在 onLoad 时自动播放。
